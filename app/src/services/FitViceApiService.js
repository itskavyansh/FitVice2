...
export class FitViceApiService {
  ...
  async generateRecipe(ingredients) {
    ...
    return this.apiCallWithRetry(apiConfig.RECIPES_ENDPOINT + '/generate', options, fallbackGenerator);
  }
  ...
  async askHealthQuestion(question, history = []) {
    ...
    return this.apiCallWithRetry(apiConfig.JARVIS_ENDPOINT + '/command', options, fallbackGenerator);
  }
...
function getGeneralAdvice(question) {
  ...
  } else if (question.includes('diet') || question.includes('nutrition') || question.includes('food')) {
